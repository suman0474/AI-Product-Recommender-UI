...
export class ThreadAwareApiClient {
   ...
   */
  private injectThreadContext(config: AxiosRequestConfig): AxiosRequestConfig {
    ...
    config.headers = {
      ...config.headers,
      'X-Main-Thread-ID': threadContext.mainThreadId,
      'X-Workflow-Thread-ID': threadContext.subThreadId || '',
      'X-Session-ID': threadContext.sessionId,
      'X-Zone': threadContext.zone || 'DEFAULT',
    };
